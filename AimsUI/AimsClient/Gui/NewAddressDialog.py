...
class NewAddressDialog(Ui_NewAddressDialog, QDialog):
    ...
    def newAddress( cls, coords, addInstance, parent=None):
        ...
        #dlg.show()
        NewAddressDialog.setDlgInstance(dlg) #Wow
        dlg.exec_()
    ...
    def setDlgInstance (self):
        ''' make the object aware of its self so it can close it self ''' 
        self.dlg = self
    ...
    def submitAddress( self ):
        ...
        if len(success) == 0:
            ...
            QMessageBox.warning(iface.mainWindow(),"Create Address Point", success)
